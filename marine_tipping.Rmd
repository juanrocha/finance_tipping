...
Then I used Lloyds (thanks Frida!) to identify owner companies. Out of the 16 878 vessels, I got companies for 2183 of them. Using Orbis I found 793 shareholders for some of the companies. Already now you can see part of our problem: each step we try to go on the chain (vessel - owner - shareholder) we lose observations (one order of magnitude), thus to what extend are results robust? Let's check them first.

## Results

```{r}
load("data/cleaned_data_231023.Rda")
...
ggplot(ggnetwork::ggnetwork(bip, arrow.gap = 0.005) ,
    ...
    geom_edges(aes(color = ownership_total), size = 0.35, alpha = 1
...
Second, most companies have only a few shareholders, just around 10 actually have 5 shareholders or more. Several of the investments are by shareholders on companies in the same countries, on the network these are loops. They are hard to see in the visualization but I scaled node size to correspond to these loops. Norway is the larger country with loops (> 200 shareholders invest in fishing companies inside Norway). For countries where there is actually foreign investments, Spain, UK and France are hot spots of shareholders, while Russia is a hot spot for receiving investments (see panel C).
...
1. How do you think we can improve the analysis?
2. What other data sets and questions can we ask?

In previous discussions we thought of using Eikon to get info on debt, who fund companies? In a preliminary analysis with Peruvian vessels and companies we did not find anything in Eikon. Given the sparsity of the current dataset, I doubt we will find much, but happy to discuss.
